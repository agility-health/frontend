	...
	namespaced: true,
  	...
  	actions: {
  		getDoctor({commit, state, rootState}){
  			...
  			axios.get(get_doctor_url(rootState.auth.user.id),  {
				 	headers: {
  		...
  		createDoctor({ commit, rootState }, doctor){
	        ...
	        this.dispatch('profile/getDoctor', {self: this})
  		...
  		getEducation({commit, state, rootState}){
  			...
  			axios.get(get_education_url(rootState.auth.user.id),  {
				 	headers: {
						'x-access-tokens': localStorage.getItem('access_token')
					}
				})
  		...
  		createEducation({ commit, rootState }, education){
  			return new Promise((resolve, reject) => {
		        ...
		        this.dispatch('profile/getEducation', {self: this})
	        })
  		},
